...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.chat = customtkinter.CTkScrollableFrame(
            self.chatFrame, label_text="Messages"
        )
        ...
        def gesturer_bt():
            label_gesturer = customtkinter.CTkLabel(
                ...
                fg_color="#63359c",
                corner_radius=20,
                text="Gesture to text -> goes here",
            )
            label_gesturer.grid(
        ...
        def speaker_bt():
            label_speaker = customtkinter.CTkLabel(
                ...
                fg_color="#35999c",
                corner_radius=20,
                text="Speech to text is going here -> goes here",
            )
            label_speaker.grid(
                row=len(self.chat.grid_slaves()) + 1,
                column=2,
        ...
        changeAppModeBtn = customtkinter.CTkButton(
            self.chatFrame, text="Change mode", image=self.photoSign, command=changeMode
        )
        ...
        self.entry = customtkinter.CTkEntry(
            self.chatFrame, placeholder_text="Text here"
        )
    ...
    def draw_landmarks(self, results, frame_rgb):
        ...
        if results.multi_hand_landmarks:
            ...
            cv2.putText(
                frame_rgb,
                f"Recognized: {self.gesture_recognizer.get_current_gesture()}",
                (80, 400),
                cv2.FONT_HERSHEY_SIMPLEX,
                1,
                (0, 255, 0),
    ...
    def start_camera(self):
        def update_camera():
            ...
            if frame is not None:
                ...
                self.camera_canvas.image = frame_tk  # type: ignore

                # Draw rectangles on the camera feed
                # Top left
                self.camera_canvas.create_rectangle(-10, 50, 120, 100, fill="green")
                self.camera_canvas.create_rectangle(
                    canvas_size // 2 - 50, 50, canvas_size // 2 + 50, 100, fill="green"
                )  # Top center
                self.camera_canvas.create_rectangle(
                    canvas_size - 120, 50, canvas_size - 50, 100, fill="green"
                )  # Top right
                text_x = (canvas_size - 120 + canvas_size - 50) / 2
                text_y = (50 + 100) / 2
                self.camera_canvas.create_text(
                    text_x, text_y, text="Your Text Here", fill="white"
                )
