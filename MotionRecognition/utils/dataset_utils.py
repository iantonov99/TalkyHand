...
from MotionRecognition.utils.landmark_utils import save_landmarks_from_video, load_array


def load_dataset():
    videos = [
        file_name.replace(".mp4", "")
        for root, dirs, files in os.walk(os.path.join("MotionRecognition", "data", "videos"))
        for file_name in files
        if file_name.endswith(".mp4")
    ]
    dataset = [
        file_name.replace(".pickle", "").replace("pose_", "")
        for root, dirs, files in os.walk(os.path.join("MotionRecognition", "data", "dataset"))
        for file_name in files
    ...
    if n > 0:
        ...
        try:
            for idx in tqdm(range(n)):
                ...
                print("Processing video: ", videos_not_in_dataset[idx], "\n")
                save_landmarks_from_video(videos_not_in_dataset[idx], )
        except Exception as e:  
            print(f"Failed to extract landmarks from video: {e}")
...
def load_reference_signs(videos):
    ...
    for video_name in videos:
        ...
        path = os.path.join("MotionRecognition", "data", "dataset", sign_name, video_name)
