    ...
    def check_and_save_letter(self):
        if self.current_letter and time.time() - self.start_time >= 1:
            ...
            if self.current_letter == "space":
                self.saved_text = self.saved_text[:-1] + " " + self.saved_text[-1:]
            elif self.current_letter == "del":
                # delete the second to last character
                self.saved_text = self.saved_text[:-2] + self.saved_text[-1:]
            elif self.current_letter == "unknown":
                pass
            else:
                # append the letter before the last character
                self.saved_text = self.saved_text[:-1] + self.current_letter + self.saved_text[-1:]
